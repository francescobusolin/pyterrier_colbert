...
class ColBERTModelOnlyFactory():
        ...
        def _text_scorer_qembs(queries_and_docs):
            ...
            with torch.no_grad():
                for qid, group in tqdm(groupby, total=len(groupby), unit="q") if verbose else groupby:
                    ...
                    query = group["query"].values[0]
                    ranking = slow_rerank_with_qembs(self.args, qembs, group["docno"].values,
                                                     group[doc_attr].values.tolist(), gpu=self.gpu)
                    for rank, (score, pid, passage) in enumerate(ranking):
                        rtr.append([qid, query, pid, score, rank])
            return pd.DataFrame(rtr, columns=["qid", "query", "docno", "score", "rank"])
    ...
    def scorer(factory, add_contributions=False, add_exact_match_contribution=False, verbose=False,
               gpu=True) -> pt.Transformer:
        """
...
class ColBERTFactory(ColBERTModelOnlyFactory):
    ...
    def _faiss_index(self):
        ...
        if len(faiss_index_files) > 1:
            warn(
                f"More than one FAISS index file matching the pattern {index_name}*.faiss found in {self.index_path}\nChoosing the first one which is at {faiss_index_path}")
    ...
    def set_retrieve(self, batch=False, query_encoded=False, faiss_depth=1000, verbose=False,
                     docnos=False) -> pt.Transformer:
        """
    ...
    def index_scorer(self, query_encoded=False, add_ranks=False, add_docnos=True, batch_size=10000) -> pt.Transformer:
        ...
        def rrm_scorer_query_embs(qid_group):
            ...
            if batch_size > 0:
                scores = rrm.our_rerank_with_embeddings_batched(qid_group.iloc[0]["query_embs"], docids, weights,
    ...
    def get_embeddings_by_token(self, tokenid, flatten=True, sample=None) -> Union[
        torch.TensorType, List[torch.TensorType]]:
        """
        ...
        all_tensors = [rrm.part_mmap[shard].mmap[shard_portion - self.segment_starts[shard]] for shard, shard_portion in
                       enumerate(partitioned_offsets) if shard_portion.shape[0] > 0]
    ...
    def ann_retrieve_score(self, batch=False, query_encoded=False, faiss_depth=1000, verbose=False, maxsim=True,
                           add_ranks=True, add_docnos=True, num_qembs_hint=32) -> pt.Transformer:
        """
        ...
        def _single_retrieve(queries_df):
            ...
            for row in iter:
                ...
                pids, final_scores = _approx_maxsim_numpy(all_scores, all_embedding_ids,
                                                          self.faiss_index.emb2pid.numpy(), qweights[0].numpy(),
                                                          score_buffer)
            ...
            rtr = pd.DataFrame(rtr,
                               columns=["qid", "query", 'docid', 'score', 'query_toks', 'query_embs', 'query_weights'])
            if add_docnos:
...
class ColbertPRF(pt.Transformer):
    ...
    def transform_query(self, topic_and_res: pd.DataFrame) -> pd.DataFrame:
        ...
        if 'doc_embs' in topic_and_res.columns:
            ...
            prf_embs = torch.cat(
                [self.pytcfactory.rrm.get_embedding(docid) for docid in topic_and_res.head(self.fb_docs).docid.values])
